...
Ablation variants (each is a config override on top of a base config):
    full                  — everything on (chain + per-stage + physics + spectral + multi-fidelity)
    no_chain              — splitting = "monolithic" (additive ensemble, the original paper's broken behavior)
    ...
    no_multifidelity      — dataset switched to non-MF version of the same data
...
VARIANTS = [
    ("full", {}),  # lie_trotter chain, per-stage decoders, current physics setting
    ("strang_splitting", {
        # Higher-order splitting (paper's novel contribution beyond Lie-Trotter)
        "model": {"predictor": {"splitting": "strang"}},
    }),
    ("no_chain", {
        # Monolithic = all predictors average; the original paper's broken chain
        "model": {"predictor": {"splitting": "monolithic"}},
    }),
    ...
    ("no_multifidelity", {
        # Switch back to vanilla darcy_3d (Tier-0 only)
        "data": {"dataset": "darcy_3d", "ndim": 3, "path": "data/darcy_3d"},
    }),
    ("no_vicreg", {
